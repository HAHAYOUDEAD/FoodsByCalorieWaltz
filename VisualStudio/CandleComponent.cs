    ...
    public class CollectibleCandleComponent : MonoBehaviour
    {
        ...
        public void ToggleLight(bool on = false, bool suppressSmoke = false)
        {
            ...
            if (on)
            {
                ...
                thisGearItem.m_DailyHPDecay = (24f / burnTimeHours) * thisGearItem.m_MaxHP * (1f / GameManager.GetExperienceModeManagerComponent().GetDecayScale()); // * m_DailyHPDecay scales based on difficulty
                thisGearItem.m_BeenInspected = false;
            }
            ...
            else
            {
                ...
                thisGearItem.m_DailyHPDecay = 0f;
        ...
        private void DoSetup()
        {
            ...
            // setup lights
            /*
            indoorsLight.AddComponent<LightTracking>();
            indoorsLight.AddComponent<LightFlickering>();
            indoorsLight.GetComponent<LightTracking>().EnableWeaponCameraLighting(true);
            indoorsLight.GetComponent<LightFlickering>().duration = 0.1f;
            indoorsLight.GetComponent<LightFlickering>().color0 = color0;
            indoorsLight.GetComponent<LightFlickering>().color1 = color1;
            indoorsLight.transform.rotation = Quaternion.identity;
            */
            ...
            //outdoorsLight.GetComponent<Light>().shadows = LightShadows.Soft;
            //outdoorsLight.GetComponent<Light>().shadowNearPlane = 0.075f;
            //outdoorsLight.transform.rotation = Quaternion.identity;

            //MelonLogger.Msg(outdoorsLight.transform.rotation.eulerAngles.ToString());
        ...
        public void Update()
        {
            ...
            {
                //MelonLogger.Msg("burned out " + Utility.GetGameObjectPath(thisGearItem.gameObject));
                destroyed = true;
